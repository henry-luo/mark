var MarkConvert = function(Mark) {
	...
	// stringify Mark as HTML
	// stringify with indentation space, may distort the HTML content
	MarkConvert.toHtml = function(object, options) {
		...
		function htm(obj, level = 0) {
			...
			for (var item of obj) {
				...
				else if (typeof item === "object") { buffer += htm(item, level+1);  hasElmt = true; }
	...
	// stringify Mark as XML; note that stringify with indentation space, may distort the XML content
	MarkConvert.toXml = function(object, options) {
		function xml(obj, level = 0) {
			if (typeof obj !== 'object') { return ''; } // skip invalid object
			if (obj.pragma()) { // XML comment or PI
				var pragma = obj.pragma();
