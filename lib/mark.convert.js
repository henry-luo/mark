var MarkConvert = function(Mark) {
	...
	MarkConvert.toXml = function(object) {
		...
		function xml(obj,indent=0) {
			...
			// print opening tag
			var buffer = tabs(indent)+ "<"+ obj.constructor.name;
			// print object attributes
			for (var prop in obj) {
				var value = obj[prop];
				// https://stackoverflow.com/questions/2647867/how-to-determine-if-variable-is-undefined-or-null
			...
			if (obj[0]) {
				...
				for (var item of obj) {
					if (typeof item === "string") { buffer += escapeStr(item); }
					else if (typeof item === "object") { buffer += xml(item,indent+1);child++ }
					else { console.log("unknown object", item); }
				}
				// print closing tag
				buffer += child?"\n"+tabs(indent):''+ "</"+ obj.constructor.name +">\n";
			} else {
				buffer += "/>";
			}
			return buffer;
