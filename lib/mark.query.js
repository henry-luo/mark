...
function isTag(node) { return typeof node === 'object' && node.constructor.name != '!comment'; }

function getParent(node) { return node[$parent]; }

function getChildren(node) {
	return node.length ? node.contents:[];
}
...
const adapter = {
	...
	getText: function(node) {
		...
		else if (node.constructor.name === '!comment') { return ''; }
		// element
		var texts = [];
